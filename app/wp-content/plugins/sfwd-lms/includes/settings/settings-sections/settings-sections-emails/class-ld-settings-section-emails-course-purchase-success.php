<?php
/**
 * LearnDash Settings Section for Email Course Purchase Success Metabox.
 *
 * @since 3.6.0
...
if ( ( class_exists( 'LearnDash_Settings_Section' ) ) && ( ! class_exists( 'LearnDash_Settings_Section_Emails_Course_Purchase_Success' ) ) ) {

	/**
	 * Class LearnDash Settings Section for Emails Course Purchase Success Metabox.
	 *
	 * @since 3.6.0
	 */
	class LearnDash_Settings_Section_Emails_Course_Purchase_Success extends LearnDash_Settings_Section {
		/**
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'learndash_settings_emails_course_purchase_success';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'learndash_settings_emails_course_purchase_success';
			...
			$this->settings_section_label = sprintf(
				...
				esc_html_x( '%s Purchase Success', 'Placeholder: Course', 'learndash' ),
				...
				'label_description' => '<div>
					...
					<ul>
						...
						<li><span>{post_title}</span> - ' . sprintf(
							...
							esc_html_x( '%s Title', 'placeholder: Course label', 'learndash' ),
							learndash_get_custom_label( 'course' )
						) . '</li>
						<li><span>{post_url}</span> - ' . sprintf(
							// translators: placeholder: Course label.
							esc_html_x( '%s URL', 'placeholder: Course label', 'learndash' ),
							learndash_get_custom_label( 'course' )
						) . '</li>
...
add_action(
	...
	function() {
		LearnDash_Settings_Section_Emails_Course_Purchase_Success::add_section_instance();
	}
);
