<?php
/**
 * LearnDash Settings Section for Email Group Purchase Success Metabox.
 *
 * @since 3.6.0
...
if ( ( class_exists( 'LearnDash_Settings_Section' ) ) && ( ! class_exists( 'LearnDash_Settings_Section_Emails_Group_Purchase_Success' ) ) ) {

	/**
	 * Class LearnDash Settings Section for Emails Group Purchase Success Metabox.
	 *
	 * @since 3.6.0
	 */
	class LearnDash_Settings_Section_Emails_Group_Purchase_Success extends LearnDash_Settings_Section {
		/**
		...
		protected function __construct() {
			...
			$this->setting_option_key = 'learndash_settings_emails_group_purchase_success';

			// This is the HTML form field prefix used.
			$this->setting_field_prefix = 'learndash_settings_emails_group_purchase_success';

			// Used within the Settings API to uniquely identify this section.
			$this->settings_section_key = 'settings_emails_group_purchase_success';
			...
			$this->settings_section_label = sprintf(
				...
				esc_html_x( '%s Purchase Success', 'Placeholder: Group', 'learndash' ),
		...
		public function load_settings_values() {
			...
			$new_settings = empty( $this->setting_option_values );
				...
				'label_description' => '<div>
					...
					<ul>
						...
						<li><span>{post_title}</span> - ' . sprintf(
							...
							esc_html_x( '%s Title', 'placeholder: Group label', 'learndash' ),
							learndash_get_custom_label( 'group' )
						) . '</li>
						<li><span>{post_url}</span> - ' . sprintf(
							// translators: placeholder: Group label.
							esc_html_x( '%s URL', 'placeholder: Group label', 'learndash' ),
							learndash_get_custom_label( 'group' )
						) . '</li>
...
add_action(
	...
	function() {
		LearnDash_Settings_Section_Emails_Group_Purchase_Success::add_section_instance();
	}
);
