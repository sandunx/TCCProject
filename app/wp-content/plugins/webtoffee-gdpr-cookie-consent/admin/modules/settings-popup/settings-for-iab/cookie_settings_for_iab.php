...
$is_consent_mode_enabled  = isset($the_options['consent_mode'])?$the_options['consent_mode']:false; 
if( $is_consent_mode_enabled ) {
        ...
        <div class="cli-row">
            <?php if ($the_options['cookie_setting_popup'] === true) : ?>
	            ...
	            <div class="cli-col-12 wt-cli-iab-preference-wrapper">
	            	...
	            	$key_arr 				= array();
					$googlevendors          = ( is_array( $array_data ) && isset( $array_data['googleVendors'] ) ) ? $array_data['googleVendors'] :array();
					?>
	                ...
	                <div class="tab-content" id="cli_cookie_vendors_content">
	                	...
	                	if(empty($vendors_arr))
	                	{
							...
							// Use error handling to check if file_get_contents is successful
						...
						if( !empty( $vendors_arr ) ) {
							foreach ( $vendors_arr as $kl => $kv ) {
								...
								$slider_switch 			 = true;
								$key 					 = isset( $vendors_arr[ $key ] ) ? $vendors_arr[$key]: '';
								$data_target             = 'third-party-vendors';
								$allowed_vendors_arr     = Cookie_Law_Info_Settings_Popup_For_IAB::get_allowed_vendor_list();
								?>
								<div class="cli-tab-section">
									<?php
									...
									<div class="cli-tab-content">
										...
										if(is_array($kv))
										{
											foreach ($kv as $p_key => $p_value) {
												...
												if( in_array( $p_key, $allowed_vendors_arr ) )
												{
													Cookie_Law_Info_Settings_Popup_For_IAB::wt_cli_generate_vendor_body_html($p_value,$p_key,$data_target,$aria_described_by,$ccpa_category_identity,$checked,$cli_enable_text,$cli_disable_text,$cli_legitimate_text,$legitimate_interest,$slider_switch,$key_arr);
												}
						...
						if( true ===  $is_consent_mode_enabled ) {
							if( empty( $googlevendors ) ) {
								Cookie_Law_Iab_Tcf::get_instance()->update_vendor_list();
								$file     = Cookie_Law_Iab_Tcf::get_vendor_list();
								// Use error handling to check if file_get_contents is successful
								$jsonData = @file_get_contents( $file );
								// Check if file_get_contents was successful
								if ( $jsonData !== false ) {
									$vendors_list_arr = json_decode($jsonData, true);
									$googlevendors    = ( is_array( $vendors_list_arr ) && isset( $vendors_list_arr['googleVendors'] ) ) ? $vendors_list_arr['googleVendors'] : array();
								}
							} 
							if( !empty( $googlevendors ) ){
								$title 					 = __('Google Ad Tech Providers','webtoffee-gdpr-cookie-consent');
								$slider_switch 			 = true;
								$providers_key 			 = 'google-vendors';
								$allowed_vendors_arr     = Cookie_Law_Info_Settings_Popup_For_IAB::get_allowed_google_vendor_list();
								$vendor_list_args_arr    = array(
									'aria_described_by'      => $aria_described_by,
									'ccpa_category_identity' => $ccpa_category_identity,
									'checked'                => $checked,
									'cli_enable_text'        => $cli_enable_text,
									'cli_disable_text'       => $cli_disable_text,
									'slider_switch'          => $slider_switch,
								);
								?>
								<div class="cli-tab-section">
									<?php
									Cookie_Law_Info_Settings_Popup_For_IAB::wt_cli_generate_header_html($title,$providers_key,$aria_described_by,$ccpa_category_identity,$checked,$cli_enable_text,$cli_disable_text,$slider_switch);
									?>
									<div class="cli-tab-content">
										<?php
										if( is_array( $googlevendors ) ) {
											foreach ( $googlevendors as $provider_key => $provider ) {
												if( 0 !== $provider_key ) {
													$p_key = $provider['id'];
													if( in_array( $p_key, $allowed_vendors_arr ) ) {
														Cookie_Law_Info_Settings_Popup_For_IAB::wt_cli_generate_google_vendor_body_html( $provider, $providers_key, $vendor_list_args_arr );
													}
												}
											}
										}
