...
class Cookie_Law_Info_Settings_Popup_For_IAB
{
	...
	public static function wt_cli_generate_google_vendor_body_html( $p_value, $key, $vendor_list_args_arr )
	{				
		if(isset($p_value))
		{
			$p_key					= isset( $p_value['id'] ) ? $p_value['id'] : '';
			$title      			= isset( $p_value['name'] ) ? $p_value['name'] : '';
			$policy_url 			= isset( $p_value['privacy'] ) ? $p_value['privacy'] : '';
			$aria_described_by      = isset( $vendor_list_args_arr['aria_described_by'] ) ? $vendor_list_args_arr['aria_described_by'] : '';
			$ccpa_category_identity = isset( $vendor_list_args_arr['ccpa_category_identity'] ) ? $vendor_list_args_arr['ccpa_category_identity'] : '';
			$checked 				= isset( $vendor_list_args_arr['checked'] ) ? $vendor_list_args_arr['checked'] : '';
			$cli_enable_text 		= isset( $vendor_list_args_arr['cli_enable_text'] ) ? $vendor_list_args_arr['cli_enable_text'] : '';
			$cli_disable_text 		= isset( $vendor_list_args_arr['cli_disable_text'] ) ? $vendor_list_args_arr['cli_disable_text'] : '';
			$slider_switch 			= isset( $vendor_list_args_arr['slider_switch'] ) ? $vendor_list_args_arr['slider_switch'] : '';
			?>
            <div class="cli-tab-section cli-sub-tab-section">
				<div class="cli-sub-tab-header cli-<?php echo $key; ?>-<?php echo $p_key; ?>">
                    <a id="wt-cli-iab-<?php echo $key; ?>-consents-item-<?php echo $p_key; ?>" tabindex="0" role="tab" aria-expanded="false" <?php echo $aria_described_by; ?> aria-controls="wt-cli-tab-<?php echo $p_key; ?>" class="cli-nav-link cli-settings-mobile" data-target="<?php echo $p_key; ?>" data-toggle="cli-toggle-tab" >
				...
				<div class="cli-sub-tab-content" id="wt-cli-iab-<?php echo $key; ?>-consents-sub-content-tab-<?php echo $p_key; ?>">
                   <?php
					if(!empty( $policy_url )) {
						?>
						<div class="cli-tab-content-privacy-policy">
							<label class="cli-privacy-policy-link-title cli-vendor-sub-title"> <?php _e('Privacy policy: ', 'webtoffee-gdpr-cookie-consent') ?></label>
							<a class="cli-privacy-policy-link" target="_blank" href="<?php echo esc_url($policy_url);?>"> <?php echo esc_html_e($policy_url); ?></a>
						</div>
						<?php
					}
				   ?>
