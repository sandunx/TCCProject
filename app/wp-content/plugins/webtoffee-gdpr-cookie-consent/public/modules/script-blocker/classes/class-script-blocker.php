					...
					if ( isset( $third_party_script[ $script_key ] ) ) {
						if ( true === $script['status'] ) {
							...
							if ( '' !== $script['category'] ) { // a category assigned
								...
								$placeholder_text                                   = sprintf( __( "Accept <a class='cli_manage_current_consent' tabindex='0'>%s</a> cookies to view the content.", 'webtoffee-gdpr-cookie-consent' ), $script['category_name'] );
							}
							if ( $scripts && is_array( $scripts ) ) {
