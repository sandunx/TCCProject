 ...
 * @version 1.9.2
 */
class Cookie_Law_Info_Package_Handler {
	...
	private $plugin_id = 'wtgdprcookieconsent'; // Unique ID for the plugin.
	...
	public function init() {
		...
		add_action( 'wp_update_plugins', array( $this, 'clear_info_cache' ), 9 ); // WP Cron.
		add_action( 'deleted_site_transient', array( $this, 'clear_info_cache' ) );
		// add_action( 'setted_site_transient', array( $this, 'clear_info_cache' ) );
	}
	...
	public function check_update( $_transient_data ) {
		...
		$status = get_option( $this->plugin_id . '_activation_status' );
		if ( 'active' !== $status ) {
			return $this->no_update_response( $_transient_data );
		}
	...
	public function get_repo_api_data() {
		...
		if ( false === $version_info || ( isset( $_GET['force-check'] ) && 1 == $_GET['force-check'] ) ) {
	 ...
	 */
	public function clear_info_cache( $transient = 'update_plugins', $value = null ) {
	...
	/**
	 * Return no update response if license is not active.
	 *
	 * @return object
	 */
	public function no_update_response( $transient ) {

		$object               = new stdClass();
		$object->new_version  = $this->version;
		$object->url          = '';
		$object->package      = '';
		$object->requires_php = '';

		$transient->no_update[ $this->name ] = $object;
