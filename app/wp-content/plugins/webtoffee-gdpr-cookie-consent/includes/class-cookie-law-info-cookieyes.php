                ...
                if (isset($response) && is_array($response)) {
                    if (isset($response['token'])) {
                        ...
                        if (isset($response['status']) && $response['status'] == 'error') {
                            ...
                            elseif ( isset( $response['error_code'] ) && 1012 == $response['error_code'] ) {
                                $api_response['status']  = false;
                                $api_response['code']    = 112;
        ...
        */
        ...
        public function set_ckyes_scan_data($option_name, $value) {
            $options = $this->get_ckyes_scan_data();
             // Ensure $options is an array before setting a value
            if (!is_array($options)) {
                $options = array();
            }
            $options[$option_name] = $value; // line no 1046
