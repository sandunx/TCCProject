	...
	class LearndashHelper implements BBLMSHelper {
		...
		public function active_lesson( $course ) {
			...
			return $wpdb->get_var( $wpdb->prepare(
				"SELECT ua.post_id FROM {$activity_table} ua INNER JOIN {$wpdb->posts} p ON ua.post_id = p.ID WHERE ua.user_id = %d AND ua.course_id = %d AND ua.activity_type != %s AND ua.activity_status = %d AND p.post_status = 'publish' ORDER BY ua.activity_updated DESC LIMIT %d",
				get_current_user_id(),
				$course,
				'course',
				0,
